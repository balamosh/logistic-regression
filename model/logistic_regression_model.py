...
from utils.metrics import average_precision_score, precision_recall_curve
...
class LogisticRegression:
    ...
    def _init_weights_normal(self, sigma=None):
        ...
        bias_size = (self.output_vector_dimension, 1)

        self.weights = np.random.normal(scale=sigma, size=weights_size)

        if self.experiment_config.weights_initialization.zero_bias:
            self.bias = np.zeros(bias_size)
        else:
            self.bias = np.random.normal(scale=sigma, size=bias_size)
    ...
    def _init_weights_uniform(self, epsilon=None):
        ...
        self.weights = np.random.uniform(-epsilon, epsilon, weights_size)

        if self.experiment_config.weights_initialization.zero_bias:
            self.bias = np.zeros(bias_size)
        else:
            self.bias = np.random.uniform(-epsilon, epsilon, bias_size)
    ...
    def __softmax(model_output: np.ndarray) -> np.ndarray:
        ...
        """
        stable_z = model_output - np.max(model_output, axis=0)
        exp_of_z = np.exp(stable_z)
        return exp_of_z / np.sum(exp_of_z, axis=0)
    ...
    def __sigmoid(model_output: np.ndarray) -> np.ndarray:
        ...
        """
        y = np.zeros_like(model_output)
        positive_mask = model_output >= 0
        exp_of_positive = np.exp(model_output[positive_mask])
        exp_of_negative = np.exp(model_output[~positive_mask])

        y[positive_mask] = 1 / (1 + exp_of_positive)
        y[~positive_mask] = exp_of_negative / (1 + exp_of_negative)
        return y
    ...
    def __get_gradient_w(self, inputs: np.ndarray, targets: np.ndarray, model_confidence: np.ndarray) -> np.ndarray:
        ...
        elif self.reg_type == RegularizationType.lasso:
            regularization_term = self.reg_coefficient_lasso * np.sign(self.weights)
        elif self.reg_type == RegularizationType.ridge:
            regularization_term = self.reg_coefficient_ridge * self.weights
        elif self.reg_type == RegularizationType.lasso_ridge:
            regularization_term = self.reg_coefficient_ridge * self.weights \
    ...
    def __update_weights(self, inputs: np.ndarray, targets: np.ndarray, model_confidence: np.ndarray):
        ...
        """
        self.weights -= self.learning_rate * self.__get_gradient_w(inputs, targets, model_confidence)
        self.bias -= self.learning_rate * self.__get_gradient_b(targets, model_confidence)
    ...
    def __target_function_value(self, inputs: np.ndarray, targets: np.ndarray,
        ...
        """
        z = z if z is not None else self.__get_model_output(inputs)

        if self.experiment_config.loss_type == LossType.softmax:
            z -= np.max(z)
            ...
            positive_mask = z_flattened >= 0
            negative_mask = ~positive_mask

            loss = np.zeros_like(z_flattened)
            loss[positive_mask] = log_values[positive_mask] + (1 - targets[positive_mask]) * z_flattened[positive_mask]
            loss[negative_mask] = log_values[negative_mask] - targets[negative_mask] * z_flattened[negative_mask]
    ...
    def compute_metrics(self, inputs: np.ndarray, targets: np.ndarray,
        ...
        return average_precision_score(targets, model_confidence[-1, :])
    ...
    def one_hot_encoding(self, targets: np.ndarray) -> np.ndarray:
        ...
        targets_ohe = np.zeros((samples_num, self.output_vector_dimension), dtype=int)
        targets_ohe[np.arange(samples_num), targets] = 1
        return targets_ohe
    ...
    def train(self, inputs_train: np.ndarray, targets_train: np.ndarray,
        ...
        """
        # Make OHE from targets when using Softmax
        if self.experiment_config.loss_type == LossType.softmax:
        ...
        for iteration in pbar:
            # Compute gradient, make optimization step and calculate train metrics
            ...
            # Calculate validation loss and metric values
