    ...
    def stop(self) -> bool:
        ...
        'Distance' criteria can be formulated as follows:
            ...
            where:
                ...
                - C_i, C_j - clusters which distance D(i, j) is measured,
        ...
        if self.config.stopping_criteria == StoppingCriteria.distance:
            ...
            return len(self.clusters.keys()) <= self.config.stopping_criteria_params.get('clusters_num_min')
        else:
    ...
    def run(self):
        ...
        with tqdm() as pbar:
            while not self.stop():
                ...
                cluster_1, cluster_2 = np.unravel_index(np.argmin(self.distances), self.distances.shape)
                ...
                # Merge clusters and remove the merged cluster from self.clusters
        ...
        # Get formed cluster labels for all data samples
